		...
		if (auto var = dynamic_cast<VariableDeclaration const*>(declaration))
		{
			...
			if (!identifierInfo.suffix.empty())
			{
				...
				else if (auto const* fpType = dynamic_cast<FunctionTypePointer>(var->type()))
				{
					...
					{
						m_errorReporter.typeError(9272_error, _identifier.debugData->location, "Variables of type function pointer only support \".selector\" and \".address\".");
						return false;
					}
					if (fpType->kind() != FunctionType::Kind::External)
					{
						m_errorReporter.typeError(8533_error, _identifier.debugData->location, "Only Variables of type external function pointer support \".selector\" and \".address\".");
						return false;
					}
					if (_context == yul::IdentifierContext::LValue)
					{
						m_errorReporter.typeError(1259_error, _identifier.debugData->location, "\".selector\" and \".address\" for external function pointers cannot be assigned to.");
						return false;
					}
				}
				else
